...
- [1. О CryptoLayer](#1-о-cryptolayer)
    ...
    - [1.2. Зачем это нужно](#12-зачем-это-нужно)
    - [1.3. Архитектура](#13-архитектура)
...
- [3. Как работает](#3-как-работает)
    - [3.1. Кратко по шагам](#31-кратко-по-шагам)
    - [3.2. Инициализация](#32-инициализация)
    - [3.3. Основной рабочий процесс](#33-основной-рабочий-процесс)
    - [3.4. Завершение работы](#34-завершение-работы)
    - [3.5. Гарантия доставки](#35-гарантия-доставки)
    - [3.6. Стабильность соединения](#36-стабильность-соединения)
    - [3.7. Пакеты](#37-пакеты)
- [4. Защита данных](#4-защита-данных)
    - [4.1. Шифрование отправляемых данных](#41-шифрование-отправляемых-данных)
    - [4.2. Шифрование файлов CryptoLayer](#42-шифрование-файлов-cryptolayer)
    - [4.3. ЭЦП и целостность данных](#43-эцп-и-целостность-данных)
    - [4.4. Обмен ключами](#44-обмен-ключами)
    - [4.5. Маскировка](#45-маскировка)
- [5. Модули](#5-модули)
    - [5.1. Где находятся существующие модули](#51-где-находятся-существующие-модули)
    - [5.2. Создание собственного модуля](#52-создание-собственного-модуля)
    - [5.3. Тестирование собственного модуля](#53-тестирование-собственного-модуля)
...
В отдельной директории создаём файлы `main.py`, `requirements.txt`, `README.md`.

В `requirements.txt` необходимо указать все зависимости которые использует модуль.

В `README.md` сделайте описание модуля, какой канал связи использует, как работает и т.д.
